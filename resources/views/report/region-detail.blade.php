...
@section('content')
    ...
    <div class="contentArea w-full max-w-full break-words">
        ...
        <div class="py-2"></div>
        <h5 class="mx-4" style="color: #8E8E8E;"><a href="/webpanel/report/product/sales/region?_token={{ request('_token') }}&from={{ request('from') }}&to={{ request('to') }}" class="!no-underline">ย้อนกลับ</a> | ขายตามภูมิศาสตร์</h5>
        <hr class="my-3 !text-gray-400 !border">

        <div class="mx-4" style="text-align: right;">

            <a href="/webpanel/report/product/sales/region/exportcsv/view?from={{ request('from') ?? ''}}&to={{ request('to') ?? ''}}&category={{ request('category') ?? ''}}&region={{ request('region') ?? ''}}"  id="exportcsv" 
            ...
            <a href="/webpanel/report/product/sales/region/exportexcel/view?from={{ request('from') ?? ''}}&to={{ request('to') ?? ''}}&category={{ request('category') ?? ''}}&region={{ request('region') ?? ''}}"  id="exportexcel"
